...
/// Stable identifier for a direct-message conversation.
///
/// Conversation IDs are generated by clients for idempotent provisioning and
/// pinned alongside the local crypto mode. They are never interpreted as MLS
/// group IDs, even though both use canonical ULID wire encodings.
...
impl TryFrom<String> for ConversationId {
    ...
    fn try_from(value: String) -> Result<Self, Self::Error> {
        let parsed = Ulid::from_string(&value).map_err(|_| DomainError::InvalidConversationId)?;
        if parsed.to_string() != value {
            return Err(DomainError::InvalidConversationId);
        }
        Ok(Self(parsed))
    }
}
...
mod tests {
    ...
    #[test]
    fn conversation_id_requires_canonical_ulid() {
        let id = ConversationId::new();
        let wire = id.to_string();
        assert_eq!(ConversationId::try_from(wire.clone()).unwrap(), id);
        assert_eq!(serde_json::to_string(&id).unwrap(), format!("\"{wire}\""));
        assert!(ConversationId::try_from(wire.to_lowercase()).is_err());
        assert!(ConversationId::try_from(String::from("conversation-1")).is_err());
    }
    use crate::UserId;
