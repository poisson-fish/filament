...
export type VoiceParticipantGatewayEvent =
  | {
...
const VOICE_PARTICIPANT_EVENT_DECODERS: {
  [K in VoiceParticipantGatewayEventType]: VoiceParticipantEventDecoder<
    Extract<VoiceParticipantGatewayEvent, { type: K }>["payload"]
  >;
} = {
...
function decodeKnownVoiceParticipantGatewayEvent<K extends VoiceParticipantGatewayEventType>(
  type: K,
  payload: unknown,
): Extract<VoiceParticipantGatewayEvent, { type: K }> | null {
  const parsedPayload = VOICE_PARTICIPANT_EVENT_DECODERS[type](payload);
  if (!parsedPayload) {
    return null;
  }
  ...
  } as Extract<VoiceParticipantGatewayEvent, { type: K }>;
}

export function decodeVoiceParticipantGatewayEvent(
  type: string,
  payload: unknown,
): VoiceParticipantGatewayEvent | null {
  if (!isVoiceParticipantGatewayEventType(type)) {
    return null;
  }

  return decodeKnownVoiceParticipantGatewayEvent(type, payload);
}