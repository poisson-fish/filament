...
- FE-02: In progress (slices 1–4 completed on 2026-02-13: extracted strict `{v,t,d}` envelope parsing into `src/lib/gateway-envelope.ts`, extracted presence-domain decoder registry/parsing into `src/lib/gateway-presence-events.ts`, extracted friend-domain decoder registry/parsing into `src/lib/gateway-friend-events.ts`, and extracted profile-domain decoder registry/parsing into `src/lib/gateway-profile-events.ts` with `src/lib/gateway.ts` rewired to consume typed decoder modules while preserving fail-closed behavior).
