...
- [x] Wire up LiveKit token updates when a user's permissions change.
- [x] Wire Gateway Events (`WorkspaceRoleCreate`, `WorkspaceChannelOverrideUpdate`, etc.) to broadcast on changes.
- **Notes**: 
...
- [x] Port the exact `apply_channel_overwrite` and `compute_base_permissions` logic to `filament-client-web` to selectively hide/show UI elements (buttons, inputs) based on effective permissions.
- **Notes**: 
...
- [x] Add explicit warnings/confirmation modals for dangerous operations.
- **Notes**: 
