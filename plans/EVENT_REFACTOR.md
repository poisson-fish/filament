...
- [x] Add CI check that compares:
  - emitted server event list
  - documented event list
  - client-supported event list
- [x] Require explicit migration entry for additive/deprecated events.
...
- 2026-02-26 (Slice 7): Added pre-staging telemetry verification hardening for the remaining Phase 7 task by strengthening `gateway_network_flow` metrics assertions to check positive counter deltas (not just metric line presence) for ingress unknown/parse-rejected reasons and channel oversized-outbound drops. Updated `docs/GATEWAY_EVENTS.md` and `docs/DEPLOY.md` with explicit staging verification steps and required counter labels/deltas for dropped/rejected gateway telemetry. Staging execution itself is still pending environment access, so the Phase 7 staging verification checkbox remains open.
- 2026-02-26 (Slice 8): Added consolidated rollout-gate integration coverage in `gateway_network_flow` (`gateway_staging_telemetry_verification_counters_increase_with_controlled_traffic`) that exercises the full staging verification matrix in one run (malformed envelope, unknown event type, invalid `subscribe`, invalid `message_create`, oversized outbound `message_create`) and asserts positive `/metrics` deltas for all required counters.
- 2026-02-26 (Slice 9): Added a repeatable staging verifier script at `infra/scripts/verify_gateway_telemetry.sh` that fail-closes on non-increasing before/after counter pairs for the rollout-required dropped/rejected metrics. Updated `docs/DEPLOY.md` and `docs/GATEWAY_EVENTS.md` to use this script as the canonical before/after delta check after generating controlled staging traffic.
